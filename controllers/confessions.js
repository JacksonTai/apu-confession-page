...
module.exports.store = async (req, res) => {
  ...
  res.redirect(
    `/confessions/create?status=submitted&id=${encodeURIComponent(
      confession.apucpId,
    )}`,
  );
};
...
module.exports.show = async (req, res) => {
  Confession.findById(req.params.id).exec(async (err, doc) => {
    ...
    const blacklistWords = docs.map((blacklistWord) => blacklistWord.content);
    ...
    if (words.length > 0) {
      for (let word of words) {
        confession.content = confession.content.replace(
          word,
          `<mark>${word}</mark>`,
        );
      }
...
module.exports.update = async (req, res) => {
  ...
  confession.photo =
    photo && photo.length > 0 ? photo.toString().split(',') : [];
  confession.video =
    video && video.length > 0 ? video.toString().split(',') : [];
